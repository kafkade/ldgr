...
# ldgr-ffi links SQLCipher's vendored OpenSSL (issue #295): those C objects are
# built for the host SDK and reference `___chkstk_darwin`, a libSystem stack-probe
# symbol that only exists from iOS 12 / watchOS 5 onward. Without an explicit
# target, cargo/rustc default to the ancient iOS 10.0 baseline, where that symbol
# is undefined and the link fails ("Undefined symbols: ___chkstk_darwin"). These
# values match the minimums declared in Package.swift so the framework links
# against the same floor the Swift package advertises. Each `cargo build --target`
# reads the env var matching its platform at link time.
