...
fn apply_remote_payload(
    ...
    match entity_type {
        ...
        // Budget/Goal have no sync wiring yet — fail closed so we never silently
        // drop financial data. Tracked by #203.
...
mod tests {
    ...
    fn unsupported_entity_type_fails_closed() {
        // Hand-build a batch with a Budget event (still unsupported) and seal it;
