...
//! exposed so integration tests can exercise reusable pieces in-process. Today
//! that is the sync transport layer (`sync`), whose `ServerTransport` is covered
//! by `tests/server_sync.rs` against a live in-process `ldgr-server`.
//!
