...
- Journal import on macOS: the Import… command (⌘I) now reads a selected hledger journal file and imports it into the vault, creating any referenced accounts (inferring their type from the account name) and adding each transaction, then refreshing the open windows. Unsupported journal syntax is reported with the offending line numbers and nothing is imported. Journal import is also exposed through the `LdgrSwift` bindings (`importJournal`) for reuse by other clients
